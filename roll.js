...
function checkAttendance(targetIds) {
  console.log('Received IDs from popup:', targetIds);

  // Find all rows in the roll
  const rows = document.querySelectorAll('tr[ng-repeat*="item.Entries"]');

  rows.forEach((row) => {
    // Locate the ID cell inside this specific row
    const idCell = row.querySelectorAll('td.tablet')[2];
